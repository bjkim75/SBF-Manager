...
import {auditLogs,changeRequestItems,changeRequests} from "../../../db/schema";
...
export async function POST(request:Request){
 try{
  ...
  const db=getDb(),now=new Date().toISOString(),requestNo=`CR-${now.slice(0,10).replaceAll('-','')}-${crypto.randomUUID().slice(0,6).toUpperCase()}`,correlationId=`AUD-${crypto.randomUUID()}`;
  ...
  await db.insert(auditLogs).values({occurredAt:now,actorName:body.requesterName,actorRole:"Requester",action:body.submit?"요청 제출":"임시 저장",targetType:"ChangeRequest",targetId:requestNo,correlationId,detailJson:JSON.stringify({itemCount:items.length,requestArea:area})});
  return Response.json({changeRequest:created,correlationId},{status:201});
