...
test("TC-07 Kiro 누락 화면이 메뉴와 라우팅에 연결된다",async()=>{const [page,data,views]=await Promise.all([read("app/page.tsx"),read("app/data.ts"),read("app/extended-views.tsx")]);for(const name of ['계층 트리','변경 전후 비교','변경이력','Jira 반영 관리','버전 관리']){assert.match(data,new RegExp(name));assert.match(page,new RegExp(name))}assert.match(views,/export function HierarchyView/);assert.match(views,/export function VersionView/);assert.doesNotMatch(data,/감사 로그/);assert.doesNotMatch(page,/AuditView|감사 로그/);assert.doesNotMatch(views,/export function AuditView/)});
